...
test <- read.csv("../../data/patchvars_desert.csv")
...
#add the 10 going the rest of the way down for mort
all_mort1 <- rbind(all_mort, c(18.001, 10, "Full Adv"), c(18.001, 10, "No Adv"), c(18.001, 10, "3/4 Adv"), c(18.001,10,"1/2 Adv"), c(18.001,10, "1/4 Adv"))
all_mort1$temps <- as.numeric(all_mort1$temps)
all_mort1$mort <-as.numeric(all_mort1$mort)
all_mort1$adv <- as.factor(all_mort1$adv)

mort_plot <- ggplot(all_mort1, aes(x = temps, y = mort, color = adv)) +
  geom_line(size = 1.5) +
  ...
  ylab("Patch Mortality (%)") +
  xlab("Mean August Temperature (C)")+
  labs(colour = "Genotype for \nThermal Tolerance")+
  scale_x_continuous(limits = c(18, 30), breaks = seq(18, 30, by = 2))+
  scale_y_continuous(limits = c(0, 100), breaks = seq(0, 100, by = 10))+
  theme_classic(base_size=18)+
  theme(legend.position = c(.75,.25)) +
  annotate(geom = "text", x = 19, y = 13.5, label = "Arrhenius Breakpoint \n temperature") +
  annotate(geom = "text", x = 29, y = 97, label = "CTmax")

ggsave("../../outputs/mort_plot.png", plot = mort_plot, dpi = 600, width = 10, height = 8)
...
#where is the max in the mean Aug stream temperature data
max(as.numeric(Fitness_aabb_V3$Fitness_aabb))

write.csv(test, "../../data/patchvars_desert.csv", row.names = FALSE)